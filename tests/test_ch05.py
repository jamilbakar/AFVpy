...
def test_ch5_average_dispersion():
    ...
    assert 0 < load("algorithm_5_7").average_dispersion(pts, [0, 0], [1, 1], [5, 5]) <= 1


def test_ch5_star_discrepancy():
    pts = [np.array([0.2, 0.2]), np.array([0.8, 0.8]), np.array([0.5, 0.5])]
    lb, ub = load("algorithm_5_8").star_discrepancy(pts, [0, 0], [1, 1], [4, 4])
    assert lb <= ub
...
def test_ch5_lcb():
    ...
    assert load("algorithm_5_11").lcb(parent, 1.0).Q == 0.1


def test_ch5_rrt_falsify(sys_disturbance, spec_threshold):
    a2, a4, a5 = load("algorithm_5_2"), load("algorithm_5_4"), load("algorithm_5_5")
...
def test_ch5_mcts_falsify(sys_disturbance, spec_threshold):
    a2, a10, a5 = load("algorithm_5_2"), load("algorithm_5_10"), load("algorithm_5_5")
