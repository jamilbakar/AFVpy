...
def test_ch12_knn_monitor():
    m = load("A12.1").KNNMonitor(DATA, k=2, gamma=1.0)
    assert m.monitor([0.05, 0.05]) and not m.monitor([5.0, 5.0])


def test_ch12_hull_monitor():
    m = load("A12.2").HullMonitor(DATA, {0: [0, 1, 2, 3], 1: [4, 5, 6]})
    assert m.monitor([0.05, 0.05]) and not m.monitor([5.0, 5.0])


def test_ch12_superlevel_set_monitor():
    from scipy.stats import multivariate_normal
    m = load("A12.3").SuperlevelSetMonitor(multivariate_normal(mean=[0, 0], cov=np.eye(2)), gamma=0.05)
