...
def test_ch7_direct_estimation(sys_disturbance, spec_threshold):
    np.random.seed(0)
    assert 0.0 <= load("algorithm_7_1").DirectEstimation(3, 200).estimate(sys_disturbance, spec_threshold) <= 1.0


def test_ch7_bayesian_estimation(sys_disturbance, spec_threshold):
    np.random.seed(0)
    post = load("algorithm_7_2").BayesianEstimation(beta(1, 1), 3, 100).estimate(sys_disturbance, spec_threshold)
    assert hasattr(post, "mean")


def test_ch7_importance_sampling(sys_disturbance, traj_distribution, spec_threshold):
    np.random.seed(0)
    est = load("algorithm_7_3").ImportanceSamplingEstimation(traj_distribution, traj_distribution, 200)
...
def test_ch7_self_importance_sampling(sys_disturbance, traj_distribution, spec_threshold):
    a9, a3 = load("algorithm_7_9"), load("algorithm_7_3")
    p = traj_distribution
