...
def test_ch11_gradient_and_integrated(sys_disturbance):
    ...
    psi = namedtuple("Spec", ["formula"])(F.Always(F.Predicate("s", lambda s: s) > 0.0))
    a2 = load("algorithm_11_2"); a2.extract = ex
    ...
    a3 = load("algorithm_11_3"); a3.extract = ex
